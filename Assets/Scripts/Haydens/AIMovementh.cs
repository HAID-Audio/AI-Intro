...
public class AIMovementh : MonoBehaviour
{
    public Transform player;
    public float chaseDistance = 3;
    public float PWPchaseDistance = 10;

    //public GameObject[] position;
    public List<GameObject> position;
    public List<GameObject> pwpposition;
    ...
    public bool IsPlayerWPInRange()
    {
        return Vector2.Distance(transform.position, playerWaypoint.transform.position) < PWPchaseDistance;
    }
    ...
    public void AIMoveTowardsPWP(Transform goal)
    {
        //if we are not near the goal
        if (Vector2.Distance(transform.position, playerWaypoint.transform.position) > minGoalDistance)
        {
            //direction from A to B
            // is B - A
            // X = B - A
            Vector2 directionToGoal = (playerWaypoint.transform.position - transform.position);
            directionToGoal.Normalize();
            transform.position += (Vector3)directionToGoal * speed * Time.deltaTime;
        }
    }
